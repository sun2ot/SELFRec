...
llm_log = Log('llm_aug', f'llm_aug_{now}')
...
file_path = '/nvme0n1p2/yelp_out/yelp_user_history.json'
...
user_history = dict(list(user_history.items())[38649:])
for user, his in tqdm(user_history.items(), desc='llm augment'):
    ...
    if len(user_preference) % 100 == 0 and len(user_preference) > 0:
        ...
        #* 持久化用户偏好数据
        with open(f'/nvme0n1p2/yelp_out/yelp_user_preference-{now}.json', 'a', encoding='utf-8') as file:
